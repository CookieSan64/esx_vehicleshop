...
--[[ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, model, plate)
    ...
    if modelPrice and xPlayer.getMoney() >= modelPrice then
        ...
        MySQL.insert('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (?, ?, ?)', {xPlayer.identifier, plate, json.encode({model = joaat(model), plate = plate})
            ...
            -- Générer une clé pour le nouveau véhicule
            MySQL.Async.execute('INSERT INTO player_keys (player_identifier, vehicle_plate) VALUES (@identifier, @plate)', {
                ['@identifier'] = xPlayer.identifier,
                ['@plate'] = plate
            }, function(rowsChanged)
                if rowsChanged > 0 then
                    -- La clé a été ajoutée avec succès
                    -- Vous pouvez afficher un message au joueur pour confirmer l'ajout de la clé
                    xPlayer.showNotification('Clé ajoutée pour le véhicule ' .. plate)
                else
                    -- La clé n'a pas pu être ajoutée
                    -- Vous pouvez afficher un message d'erreur au joueur
                    xPlayer.showNotification('Erreur lors de l\'ajout de la clé')
                end
            end)
